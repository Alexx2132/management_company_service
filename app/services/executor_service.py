...
from app.services.executor_availability import get_executor_availability_state
...
class ExecutorService:
    ...
    def create_specialty(self, payload: SpecialtyCreate):
        existing = (
            ...
            .filter((Specialty.code == payload.code) | (Specialty.name == payload.name))
            .first()
        )
        if existing:
    ...
    def create_executor(self, payload: ExecutorCreateRequest):
        existing_login = self.user_repo.get_by_login(payload.login)
        if existing_login:
            raise HTTPException(status_code=409, detail="Executor login already exists")
    ...
    def update_executor(self, executor_id: int, payload: ExecutorUpdateRequest):
        ...
        profile.user.full_name = f"{profile.last_name} {profile.first_name}".strip()
    ...
    def update_own_executor_profile(self, user: User, payload: ExecutorUpdateRequest):
        ...
        if any(key in data for key in ["first_name", "last_name"]):
            profile.user.full_name = f"{profile.last_name} {profile.first_name}".strip()
